...
**为什么不使用子网号的全0,和全1(在不变长子网中)**

+ 全0子网的网络地址和划分子网前的网络地址重合
+ 全1子网的广播地址和划分子网前的广播地址重合
这种重合带来了混乱,所以不使用全0,全1的子网号

后来又使用了VLSM,全0,全1又可以使用了
(全0和全1子网号的使用需要路由器的支持..
...
$
10.0.0.0-10.255.255.255\quad 10.0.0.0/8,24位块\\\
172.16.0.0-172.31.255.255\quad 172.16.0.0/12,20位块\\\
192.168.0.0-192.168.255.255\quad 192.168.0.0/16,16位块
$
把使用这样的专用IP地址的互联网称为**专用互联网/本地互联网**
...
+ 网络地址转换network address translation
	- NAT地址转换表
		+ 出,换源地址;入,换目的地址;
	- 当NAT路由器具有n个全球IP地址时,专用网内最多可以同时具有n个主机接入因特网
	使专用网内数量较多的主机,轮流使用NAT路由器数量有限的全球IP地址
+ 网络地址与端口号转换NAPT
	- 把运算层的端口号也用上
